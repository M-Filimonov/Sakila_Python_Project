...
def search_by_keyword(db, root):
    try:
        ...
        handle_error_with_recommendation("Unexpected Error", str(e))

def search_by_category_and_year(db):
    try:
        cat = get_info_from_db(db, "category_list")
        res = display_table(cat, 'Choice a category:')
        category = res.get('category')
        yr = get_info_from_db(db, "year_list", (category,))
...
def search_by_actor(db):
    try:
        act = get_info_from_db(db, "actor_list")
        res = display_table(act, 'Choice an Actor:')
        ...
        if actor_id:
            ...
            db.insert_query_log('film_by_actor', f'{res.get('FirstName')} {res.get('LastName')}')
    except Exception as e:
        handle_error_with_recommendation("Unexpected Error", str(e))

def show_popular_queries(db):
    try:
        pq = get_info_from_db(db, "show_popular_queries")
        display_table(pq, 'Popular queries to Database:  | >>  to EXIT: (<Double-Click> or <Enter>)')
    except Exception as e:
        handle_error_with_recommendation("Unexpected Error", str(e))
